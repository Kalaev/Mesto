const likeButtons = document.querySelectorAll(".element__button-like"); //Объявил констанцию лайкающих кнопок, что она в html называется классом like__button//
...
function like(){//работать лайк будет так://
    if(this.classList.contains('element__button-like_active')){//если кликнув там будет лайк актив,... //
        this.classList.remove("element__button-like_active");//...remove его удалит//
    }else{
        this.classList.add("element__button-like_active");//в иных случаях(когда не актив)add сделает его актив//
    }
}
